  ...
  return (
    <React.Fragment>
        ...
        {Object.entries(user).map(([key, value]) => {
          ...
          return (
            <div
              ...
              <h1 className='text-primary-text-60/50 tracking-wider text-xs uppercase font-sans font-bold'>{names[key]}</h1>
              <div className='w-full flex flex-row gap-4'>
                <h3 className='break-all text-primary-text font-mono text-left'>{value}</h3>
                <div title='copy value to clipboard' className='opacity-0 group-hover:opacity-100 absolute right-2 bottom-2 cursor-copy'>
                  <svg xmlns='http://www.w3.org/2000/svg' className='h-5 w-5 text-primary-text-30 group-hover:text-primary-text-60' viewBox='0 0 20 20' fill='currentColor'>
      ...
      <div className='w-full items-center flex flex-wrap justify-center gap-4 p-2'>
        <button onClick={() => {}} className='col-span-2 w-fit bg-secondary-dark/80 hover:bg-secondary-dark  text-secondary-text font-mono text-sm hover:underline hover:font-bold hover:drop-shadow-lg px-4 py-2 rounded-md'>
          Re-generate Secret Key
        </button>
        <button onClick={() => {}} className='col-span-2 w-fit bg-secondary-dark/80 hover:bg-secondary-dark  text-secondary-text font-mono text-sm hover:underline hover:font-bold hover:drop-shadow-lg px-4 py-2 rounded-md'>
          Re-issue API Key & Secret
        </button>
        <button onClick={() => {}} className='col-span-2 w-fit bg-red-500/80 hover:bg-red-500 text-secondary-text font-mono text-sm hover:underline hover:font-bold hover:drop-shadow-lg px-4 py-2 rounded-md'>
          Delete API User
        </button>
      </div>
    </React.Fragment>
